...
async function exportDocumentToHtml(documentName: string): Promise<string> {
  ...
  const fullHtml = [
      ...
      '/* Nested (Tab-indented) blocks that are not list items are flattened by',
      '   blocksToHTMLLossy() into siblings carrying only a data-nesting-level',
      '   attribute, so their indentation has to be restored here. Nested lists',
      '   already indent through their own <ul>/<ol> padding-left. */',
      '[data-nesting-level="1"] { margin-left: 2em; }',
      '[data-nesting-level="2"] { margin-left: 4em; }',
      '[data-nesting-level="3"] { margin-left: 6em; }',
      '[data-nesting-level="4"] { margin-left: 8em; }',
      '[data-nesting-level="5"] { margin-left: 10em; }',
      '[data-nesting-level="6"] { margin-left: 12em; }',
      'li[data-nesting-level] { margin-left: 0; }',
      'code {',
